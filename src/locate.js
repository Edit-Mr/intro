...
stlLoader.load("./models/EM.stl", function (geometry) {
  ...
  effect.domElement.addEventListener("mousemove", event => {
    ...
    camera.rotation.x = ${camera.rotation._x};
    camera.rotation.y = ${camera.rotation._y};
    camera.rotation.z = ${camera.rotation._z};`);
  });
  ...
  // camera.position.x = -2.1797208198865823;
  // camera.position.y = 0.7456858904043902;
  // camera.position.z = 1.259941607611314;
  // camera.rotation.x = -0.10187680375576491;
  // camera.rotation.y = -1.3965809813885177;
  // camera.rotation.z = -0.10034508895856975;
});
